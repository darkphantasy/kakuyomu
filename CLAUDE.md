...
- **Web アプリ(取得インターフェース)**: `doGet` が `index.html` を返し、クライアントから `google.script.run` で `WebApp.js` の `web*` 関数を呼ぶ。**取得ロジック本体には手を入れず**、既存部品(`isRunActive_`/`enqueueWork_`/`prepareFetch`/`prepareContinuation`/`batchStartNext`/`ensureTriggerAfter`)の組み合わせで実装している。要点:
  ...
  - **ポーリングは取得操作を実行している間だけ**(`scheduleNextRefresh`): `webGetState` の応答で `running.active` が true の間だけ `POLL_MS=7000` 後に次回を予約する自走方式(`setInterval` ではなく `refresh()` の成功時に自分自身が次回を `setTimeout` で予約する)。**待機中(`active:false`)は次回を予約せず、ポーリングが完全に止まる**。誰も見ていない・何も動いていない間まで GAS の実行枠や UrlFetchApp クォータを消費しない設計上の判断(タブを開いたままバックグラウンドに置いても、待機中なら負荷が発生しない)。
    - 再開のきっかけはボタン操作(`call()` が成功時に直接 `refresh()` を呼ぶので、取得を開始すればそこで `active` になりポーリングが再開する)・「今すぐ更新」ボタン・ページの再読み込みのみ。
    - 取得失敗時(サーバーエラー等)は `active` かどうか不明なため `POLL_MS` 後に1回再試行する(ポーリングが完全に止まりきらないようにする保険)。
    - **Web UI の外(GAS エディタから直接関数実行等)から取得を開始した場合は、画面をリロードするかボタン操作するまで気づかない**(待機中はそもそもポーリングしていないため)。この用途は現状 Web UI に一本化されているため許容している。
    - **バックグラウンドタブ対策**(`visibilitychange`): ブラウザは非表示タブの `setTimeout` を間引くため、実行中にタブを離れると、戻ってきたときにはバッチが完了していても気づけないまま止まっていることがある。そこで**タブが隠れた瞬間に `running.active` だったかどうかを覚えておき、再表示された時にそれが true だった場合だけ** `refresh()` を1回実行して追いつく。**何も実行していない時にタブを切り替えても何も起きない**(=「実行中だけポーリングする」方針を破らない。ここで無条件に `refresh()` すると、離席中に一括で何件も完了した作品の次話・サイズ問い合わせがタブを見せるたびにまとめて走ってしまうため、明示的に「隠れた時点で実行中だったか」に条件を絞っている)。
  ...
  - **ドキュメントサイズ表示**: 進捗の目安(参考値。実文字数と厳密には一致しない)。`webGetDocSizes(docIds)` は定期ポーリングの `webGetState` には含めない別経路。クライアント側は `docSizes`(docId→バイト数のフラットな連想配列。`hasOwnProperty` で「未取得」と「0バイト判明」を区別する)を持つ。更新のトリガーは2つ:
    ...
    - `refreshChangedWorks`: 作品ごとの `updatedAt` を `knownUpdatedAt`(workId→前回ポーリング時点の値)と比較し、**変化した作品の docId だけ**取り直す。1作品完了するたびにその作品ぶんだけ更新される(一覧から消えた workId の記録もここで掃除する。後述の次話もここで一緒に取り直す)。**ここでの `fetchSizes` 呼び出しは次話側の dedup 対象にしない**(常に全 docId を明示的に呼ぶ)。次話の判定は見出しが見つかった時点で走査を打ち切るため、読書位置より後ろの巻(＝今回の取得で実際に追記された巻であることが多い)は `sizes` に含まれないことがあり、そこを取りこぼすと直前まで悩んだ「一括続き取得完了後にサイズが更新されない」不具合が再発する。
  ...
  - **次話表示**(一覧の「次話」列。旧称「既読」→値の実態は「読了済み」ではなく「読み残しの最前線」なので改称): ユーザーは読了ぶんをドキュメント**先頭から削除して上書き保存する**運用なので、「先頭に残っている最初の `[NNN]`」が今読んでいる(または次に読む)話にあたる。`webGetReadingProgress({workId: [docId,...]})` が判定し、`{progress: {workId: 話数 or 'latest'}, sizes: {docId: バイト数}}` を返す(`progress` は判定できなかった作品を**キーごと返さない**。クライアントは未取得として次回また問い合わせる)。**`'latest'` は画面上「最新」と表示**(未読なし、の意)。`sizes` は判定の過程で実際に開いた(＝`DriveApp.getFileById` した)ドキュメントぶんだけ含む — 見出しが早めに見つかって走査を打ち切った分冊より後ろは含まれない。これは `webGetDocSizes` と**同じファイルを二重に開かない**ための副産物で、クライアントはこれを `docSizes` にまず反映し、それでも不明な docId だけ改めて `webGetDocSizes` に問い合わせる(`fetchProgressChunk` 内の `remain` 計算)。
    ...
    - 分冊は `docIds` を古い順に見て、**削除済み(`isTrashed`/例外)と見出しが残っていない分冊は飛ばして次を見る**。どれにも見出しが無ければ `'latest'`(未読なし・最新話まで読了)。
