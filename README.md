...
- **既読話数の表示**(「既読」列): 読了したぶんをドキュメントの先頭から削除していく運用に合わせ、**先頭に残っている最初の話番号**を「いま読んでいる話」として表示する。見出しが1つも残っていなければ「最新」(最新話まで読了)。判定中は「…」
...
- **一覧の並び替え・絞り込み**: 「作品」「既読」「話数」「最終更新」の見出しをクリックすると並び替え(もう一度押すと昇順/降順が反転)。作品名での絞り込み入力欄に加え、**「未読のみ表示」チェックボックス**で既読話数が「最新」(=未読なし)と確定している作品を一覧から除外できる(判定中の作品は確定するまで表示されたままになる)。並び替え・絞り込みはブラウザ側だけで処理するため、自動更新で設定が失われることはない
