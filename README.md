...
ブラウザで開く Web 画面から取得を操作できます。作品URLを貼る欄に加え、**保存済み作品の一覧が表示され、行ごとに「続き取得」「削除」ボタン**が並びます。**取得を実行している間**は進捗と順番待ちの状況が数秒おきに自動更新されます(待機中は自動更新が止まり、GASの実行枠を無駄に消費しません。ボタン操作や再読み込みで最新の状態を取得できます)。
