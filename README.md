...
- **ファイル名短縮**: 新規ドキュメント作成時、**Driveのファイル名にのみ**短縮したタイトルを使う(書籍化情報やサブタイトルが長く冗長な作品名の可読性向上が目的)。ルールベースで、「〜サブタイトル〜」形式の部分や【】等の注記を除去し、なお長ければ読点区切りで短縮する。**ドキュメント本文の見出し・索引・取得履歴は常に元のタイトルのまま**。デフォルトON、操作パネルからON/OFF切り替え可能(`SHORT_FILENAME` プロパティ)
