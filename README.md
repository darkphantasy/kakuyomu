...
- **候補から選んで登録**: 「候補から選んで登録」カードにあるブックマークレット(📥 のリンク)をブラウザのブックマークバーへドラッグしておき、ログイン状態のままカクヨムの[閲覧履歴](https://kakuyomu.jp/my/antenna/reading_histories)または[未読あり一覧](https://kakuyomu.jp/my/antenna/works?serial_status=all)を開いた状態でクリックすると、作品一覧(URL・タイトル・未読話数・全話数)が JSON でダイアログに表示される。それを「候補から選んで登録」欄に貼り付けて「読み込み」を押すと候補一覧が表示され、チェックした作品だけ「選択した作品を登録」で一覧に追加できる。未読話数が分かる候補は、既読ぶん(全話数−未読話数)を取得済み扱いにして続き取得の起点にする(分からない候補は全話取得済み扱いになる)。複数ページぶん貼り付けても作品IDで重複しない。認証はブラウザのログイン状態をそのまま使うだけで、この処理でカクヨムのログイン情報がスクリプト側に渡ることはない。ドラッグできない環境向けに、同じ処理を `kaku_scraping/tools/extract_antenna_list.js` としてコンソールへ直接貼り付ける方法も用意している
